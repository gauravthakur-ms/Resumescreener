...
from endpoints.resume_conversion import (
    handle_convert_upload, handle_conversion_status, handle_get_conversion,
    handle_update_conversion, handle_conversion_download, handle_list_conversions,
    handle_delete_conversion,
)
from processing.queue_processor import process_resume_message
from processing.conversion_queue_processor import process_conversion_message
...
@require_auth
def post_convert(req: func.HttpRequest) -> func.HttpResponse:
    """POST /api/convert — Upload a resume for conversion to LTM format."""
    return handle_convert_upload(req)


@app.route(route="convert/{conversion_id}/status", methods=["GET"], auth_level=func.AuthLevel.ANONYMOUS)
@require_auth
def get_conversion_status(req: func.HttpRequest) -> func.HttpResponse:
    """GET /api/convert/{conversion_id}/status — Poll conversion progress."""
    return handle_conversion_status(req)


@app.route(route="convert/{conversion_id}", methods=["GET"], auth_level=func.AuthLevel.ANONYMOUS)
@require_auth
def get_conversion_detail(req: func.HttpRequest) -> func.HttpResponse:
    """GET /api/convert/{conversion_id} — Get extracted data for preview."""
    return handle_get_conversion(req)


@app.route(route="convert/{conversion_id}", methods=["PUT"], auth_level=func.AuthLevel.ANONYMOUS)
@require_auth
def put_conversion(req: func.HttpRequest) -> func.HttpResponse:
    """PUT /api/convert/{conversion_id} — Update extracted data and regenerate DOCX."""
    return handle_update_conversion(req)


@app.route(route="convert/{conversion_id}/download", methods=["GET"], auth_level=func.AuthLevel.ANONYMOUS)
@require_auth
def get_conversion_download(req: func.HttpRequest) -> func.HttpResponse:
    """GET /api/convert/{conversion_id}/download — Download generated DOCX."""
    return handle_conversion_download(req)


@app.route(route="conversions", methods=["GET"], auth_level=func.AuthLevel.ANONYMOUS)
@require_auth
def get_conversions_list(req: func.HttpRequest) -> func.HttpResponse:
    """GET /api/conversions — List user's conversion history."""
    return handle_list_conversions(req)


@app.route(route="convert/{conversion_id}", methods=["DELETE"], auth_level=func.AuthLevel.ANONYMOUS)
@require_auth
def delete_conversion_by_id(req: func.HttpRequest) -> func.HttpResponse:
    """DELETE /api/convert/{conversion_id} — Delete a conversion."""
    return handle_delete_conversion(req)


# ============================================================
# Conversion Queue Trigger
# ============================================================

@app.queue_trigger(
    arg_name="msg",
    queue_name="resume-conversion-queue",
    connection="AzureWebJobsStorage",
)
def process_conversion_queue(msg: func.QueueMessage):
    """Queue trigger — processes one resume conversion per message."""
    try:
        raw = msg.get_body().decode("utf-8")
        try:
            decoded = base64.b64decode(raw).decode("utf-8")
        ...
        log_with_context(logger, "INFO", "Conversion queue message received",
                        stage="conversion_queue_trigger")
        process_conversion_message(message_body)

    except Exception as e:
        log_with_context(logger, "ERROR", f"Conversion queue processing failed: {e}",
                        stage="conversion_queue_trigger")
